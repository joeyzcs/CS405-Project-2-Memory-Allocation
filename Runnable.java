...
public class Runnable {
	...
	private static void runFirstFit(List<Process> procList, ContiguousMemoryAllocator mmu, Scanner scanner) {
		int sysTime = 0;
		...
		while (remainingProcs > 0) {
			...
			for (Process p : procList) {
				if (p.getIsAllocated() && !p.getIsTerminated() && p.getStartTime()*1000 + p.getTime() <= (sysTime * 1000)) {
